...
# Read config from ETL_Project/config.py
config_path = os.path.join(os.path.dirname(__file__), 'ETL_Project', 'config.py')
...
DB_USER = 'root'
DB_PASS = ''
DB_NAME = 'aw_datawarehouse'

for line in config_content.split('\n'):
    if line.startswith('DB_HOST'):
        DB_HOST = line.split('=')[1].strip().strip("'").strip('"')
        if 'os.environ' in line:
            DB_HOST = 'localhost'
    elif line.startswith('DB_PORT'):
        port_val = line.split('=')[1].strip().strip("'").strip('"')
        if port_val.isdigit():
            DB_PORT = int(port_val)
    elif line.startswith('DB_USER'):
        DB_USER = line.split('=')[1].strip().strip("'").strip('"')
        if 'os.environ' in line:
            DB_USER = 'root'
    elif line.startswith('DB_PASS'):
        DB_PASS = line.split('=')[1].strip().strip("'").strip('"')
        if 'os.environ' in line:
            DB_PASS = ''
